...
// Assumes you already have: final authProvider = StreamProvider<User?>((ref) => FirebaseAuth.instance.authStateChanges());

final profileFutureProvider = FutureProvider<Map<String, dynamic>>((ref) async {
  // Wait for auth to settle; avoids "User not logged in" during app start.
  final user = await ref.watch(authProvider.future);
  if (user == null) {
    // No user yet (logged out or still starting) → return empty profile
  ...
  final data = doc.data();
  if (data == null) {
    // Profile doc missing → return empty (don’t throw)
    return <String, dynamic>{};
  }

  // Push related UI state safely
