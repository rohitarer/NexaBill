...
class _BillContainerState extends ConsumerState<BillContainer> {
  StreamSubscription<DocumentSnapshot<Map<String, dynamic>>>? _billSub;
  ...
  final bool _hasResetAfterSeal = false;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (adminUid == null || adminUid.isEmpty) {
        debugPrint("⛔️ No admin UID, skipping _loadBillData()");
        return;
      }
      _loadBillData();
    });
  ...
  Future<void> _loadBillData() async {
    try {
      ...
      if (selectedAdminUid == null || selectedAdminUid.isEmpty) {
        debugPrint("⛔️ No admin UID, not loading/creating bill.");
        return;
      }
      ...
      if (BillData.customerId.isNotEmpty && BillData.billNo.isNotEmpty) {
        ...
        _billSub = billStream.listen((billSnapshot) {
          if (!billSnapshot.exists || !mounted) return;
        ...
        AnimatedPositioned(
          ...
          child: SizedBox(
            ...
            child: DraggableScrollableSheet(
              key: const PageStorageKey('bill_sheet'), // 👈 preserves position
  ...
  Widget _buildCustomerDetails() {
    ...
    final shouldUpdate =
        BillData.customerId.isNotEmpty && BillData.billNo.isNotEmpty;
    ...
    children:
        widget.billItems.map((item) {
          ...
          return Dismissible(
            ...
            key: ValueKey(ref.watch(selectedAdminUidProvider) ?? 'none'),
  ...
  Widget _buildBillSummary(bool isDarkMode) {
    ...
    final espCount = ref.watch(bluetoothItemCountProvider); // ESP32 basket
    // final billCount = widget.billItems.length;
    final billCount = totalQuantity;

    final isMatching = billCount == espCount;
    // 👇 Correct place to watch camVerification
    final camVerification = ref.watch(camVerificationProvider);
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text(
        ...
        if (camVerification) ...[
          // ✅ Original Bill vs Basket compare UI (unchanged)
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Row(
          ...
          // 🆕 Simple, single-line total items when cam verification is OFF
          Row(
            children: [
              const Icon(Icons.receipt_long, size: 20, color: Colors.blue),
              const SizedBox(width: 6),
  ...
  Widget _buildPaymentDetails(bool isDarkMode) {
    ...
    final hasValidBill =
        BillData.billNo.isNotEmpty && BillData.customerId.isNotEmpty;
...
// import 'package:nexabill/providers/profile_provider.dart';
// import 'package:nexabill/ui/screens/customer_home_screen.dart'
//     hide selectedAdminUidProvider, scannedProductsProvider;
// // import 'package:nexabill/ui/screens/customer_home_screen.dart'
// //     hide scannedProductsProvider, selectedAdminUidProvider;
// import 'package:nexabill/ui/widgets/bill_otp_handler.dart';
// import 'package:nexabill/ui/widgets/cahier_info_handler.dart';
// import 'package:nexabill/ui/widgets/verification_stamp.dart';
...
//       // final currentUser = FirebaseAuth.instance.currentUser;
//       if (currentUser == null) {
//         debugPrint("❌ No Firebase user found. Aborting bill load.");
//         return;
//       }
...
//             child: Text(
//               "⚠️ Waiting for bill to be initialized...",
