...
class _BottomInputBarState extends ConsumerState<BottomInputBar> {
  ...
  Widget build(BuildContext context) {
    ...
    final int totalBillQuantity = scannedProducts.fold<int>(
      0,
      (sum, item) => sum + ((item["quantity"] ?? 1) as int),
    );

    // 🚫 Lock pay button after first payment
  final bool paymentLocked = (BillData.amountPaid > 0) || (BillData.otp.isNotEmpty);

  // ✅ Enable logic
  final bool isPayButtonEnabled = !paymentLocked && (
    camVerification
