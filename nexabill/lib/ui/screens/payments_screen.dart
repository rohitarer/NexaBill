/// Updated PaymentsScreen with proper state management and provider usage
/// Fixes: Empty bill list, disposed provider errors, and reactivity.
library;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return profileAsync.when(
      data: (profile) {
        ...
        ref.listenManual(paymentsProvider, (previous, next) {}, fireImmediately: true);

        WidgetsBinding.instance.addPostFrameCallback((_) {
          ref.read(paymentsProvider.notifier).fetchCustomerPayments(user.uid);
        });

        final sortedPayments = [...payments]
          ..sort((a, b) {
            final aNum = int.tryParse(a['billNo'].toString().replaceAll("BILL#", "")) ?? 0;
            final bNum = int.tryParse(b['billNo'].toString().replaceAll("BILL#", "")) ?? 0;
            return bNum.compareTo(aNum);
          });

        return Scaffold(
          appBar: role == 'customer' ? AppBar(title: const Text('Payment History')) : null,
          body: sortedPayments.isEmpty
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final totalGST = products.fold(0.0, (sum, item) {
                      final gst = double.tryParse(item['gst'].toString()) ?? 0.0;
                      final price = double.tryParse(item['price'].toString()) ?? 0.0;
                      final qty = int.tryParse(item['quantity'].toString()) ?? 1;
                      return sum + (gst * price * qty) / 100;
                    });

                    final balance = (totalAmount + totalGST) - (bill['amountPaid'] as num).toDouble();
                            ...
                            Row(
                              ...
                              children: [
                                Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text("Bill No: ${bill['billNo']}", style: const TextStyle(fontSize: 12)),
                                    Text("Mobile: ${bill['customerMobile']}", style: const TextStyle(fontSize: 12)),
                                  ],
                                ),
                                Column(
...
//           //         : null,
//           appBar:
//               role.toString().toLowerCase() == 'customer'
//                   ? AppBar(title: const Text('Payment History'))
//                   : null,
