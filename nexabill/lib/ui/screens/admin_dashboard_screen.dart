...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  Future<void> _fetchBillStats() async {
    ...
    try {
      ...
      // // final adminMart = adminDoc.data()?['mart'] ?? '';
      // final adminMart =
      //     adminDoc.data()?['martName'] ?? adminDoc.data()?['mart'] ?? '';

      // final querySnapshot = await FirebaseFirestore.instance
      //     .collectionGroup('my_bills')
      //     .where('martName', isEqualTo: adminMart)
      //     .get(const GetOptions(source: Source.serverAndCache));
  ...
  //       final data = doc.data();
  //       final role = data['role']?.toString().toLowerCase();
  //       final mart = data['mart'] ?? '';

  //       if (role == 'cashier' && mart == adminMart) {
  //         debugPrint("🧾 Raw cashier doc: \${doc.data()}");
  ...
  //         final phone = data['phoneNumber'] ?? "N/A";
  //         final counter = data['counterNumber'] ?? "N/A";
  //         final martName = data['martName'] ?? mart;
  //         final profileImage = data['profileImageUrl'];
  ...
  //         debugPrint(
  //           "👤 Cashier found: \$fullName (\$email) | Mart: \$martName",
  //         );
  //       }
  //     }
  ...
  Widget _buildGraphCard(Map<String, int> dataMap, String title) {
    ...
    // ✅ Calculate dynamic maxY and interval inside the method
    final maxYValue =
        dataMap.values.isEmpty
            ? 0
            : (dataMap.values.reduce((a, b) => a > b ? a : b) * 1.2).ceil();
