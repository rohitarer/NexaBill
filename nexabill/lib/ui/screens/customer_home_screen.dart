...
class _CustomerHomeScreenState extends ConsumerState<CustomerHomeScreen>
  ...
  void initState() {
    super.initState();
    // react to admin UID changes (optional)
    // ref.listen<String?>(selectedAdminUidProvider, (prev, next) {
    //   // could clear/prepare UI here if desired
    // });
  ...
  void onMartSelected(String martName) async {
    ...
    // Hard reset bill + providers
    BillData.resetBillData(); // ensure it clears billNo & sets adminUid
    BillData.adminUid = adminUid ?? ""; // 👈 explicit
    ...
    // Cancel any existing BT count to avoid stale values (optional)
  ...
  Widget build(BuildContext context) {
    ...
    // inside build() where you render BillContainer
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          if (selectedMart != null && selectedAdminUid != null)
            BillContainer(
              key: ValueKey(
                selectedAdminUid,
              ), // 👈 force a new instance when mart/admin changes
